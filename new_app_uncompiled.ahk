...
install:
{
	...
	WinWait, Logitech Backup and Restore Service
...
backup:
{
	...
	WinWait, Backing up Logitech Settings
	WinWaitClose ; Use the window found by WinWait.
...
restore:
{
	...
	WinWait, Restoring Logitech Settings
	WinWaitClose ; Use the window found by WinWait.
	MsgBox, Settings and Profiles Restored
}
Return
...
startup:
{
	...
	WinWait, schedule tasks.bat
	WinWaitClose ; Use the window found by WinWait.
	MsgBox, Created a Startup Task. On Windows launch your settings will be backed up in the background. 
	}
	else IfMsgBox, No
		return
}
